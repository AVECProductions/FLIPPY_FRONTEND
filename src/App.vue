...
<template>
  <div id="app" class="flex flex-col min-h-screen bg-[#1C1C1E] text-gray-100 w-full">
    <!-- Navigation Bar -->
    <nav class="sticky top-0 z-10 border-b border-gray-800 bg-[#2C2C2E] w-full">
      <div class="w-full max-w-[95%] mx-auto px-4">
        <div class="flex items-center justify-between h-16">
          <!-- Logo -->
          <div class="flex items-center">
            <router-link to="/" class="text-xl font-semibold text-white">FlippyBot</router-link>
          ...
          <div class="ml-4 hidden md:flex items-center md:ml-6">
            ...
            <div v-else class="relative ml-3">
              <button 
                ...
                class="flex items-center text-sm bg-gray-800 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500"
                id="user-menu-button"
                aria-expanded="false"
                aria-haspopup="true"
              >
              ...
              <div 
                v-if="isProfileOpen" 
                class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-[#2C2C2E] ring-1 ring-black ring-opacity-5 focus:outline-none z-50"
                role="menu"
                aria-orientation="vertical"
                aria-labelledby="user-menu-button"
                tabindex="-1"
              >
                <div class="block px-4 py-2 text-sm text-gray-300 border-b border-gray-700">
      ...
      <div class="md:hidden" :class="{'block': mobileMenuOpen, 'hidden': !mobileMenuOpen}">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          <router-link 
            ...
            :class="[$route.path === '/scanners' ? 'bg-gray-700 text-white' : '']"
          ...
          <router-link 
            ...
            to="/listings" 
            class="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white"
            :class="[$route.path === '/listings' ? 'bg-gray-700 text-white' : '']"
            @click="mobileMenuOpen = false"
          >
            Listings
          </router-link>
          <div v-if="isAuthenticated" class="border-t border-gray-700 pt-4 pb-3">
          ...
          <router-link
            ...
            to="/login"
            class="block px-3 py-2 rounded-md text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white"
            :class="[$route.path === '/login' ? 'bg-gray-700 text-white' : '']"
            @click="mobileMenuOpen = false"
          >
            Login
          </router-link>
